    ...
    <div className="w-full">
      <div className="flex items-center justify-between mb-5 px-4 md:px-10">
        <div className="flex items-center gap-3">
          <span className="w-1.5 h-7 rounded-full bg-amber-400 shadow-[0_0_8px_rgba(251,191,36,0.6)]" />
          <h3 className="text-xl md:text-2xl font-bold text-slate-700 tracking-tight">{label}</h3>
        </div>
        <div className="flex gap-2">
          <button
            onClick={() => scrollBy("left")}
            className="w-9 h-9 rounded-full bg-amber-100 hover:bg-amber-200 text-amber-700 flex items-center justify-center font-bold text-lg transition-all hover:scale-110"
          >&#8249;</button>
          <button
  ...
  return (
    <section id="qualitywall" className="relative w-full py-20 bg-slate-50 overflow-hidden">
      ...
      <div className="max-w-[100vw]">
        <ScrollableCarousel images={qualityWallImages} label="Quality & Safety Visuals" />
      </div>
    </section>
  );
}
